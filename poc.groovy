pipeline {
  ...
  stages {
    stage('Determine target environment URL') {
      steps {
        script {
          ...
          // normalize like: "Stage 1" -> "stage1", "DEV" -> "dev"
          def key = (params.ENVIRONMENT ?: '')
                      .toLowerCase()
                      .replaceAll(/\s+/, '')
    ...
    stage('Create deployment manifest file') {
      steps {
        script {
          // Put your JSON exactly as JSON (safe & readable)
          def manifestJson = '''{
            "add": {
              "dynamics": [
                {
          ...
          // Save and prettify (optional)
          writeFile file: env.MANIFEST_FILE, text: manifestJson
          def pretty = groovy.json.JsonOutput.prettyPrint(manifestJson)
    ...
    stage('Extract Flowon commands (solutions)') {
      steps {
        script {
          // Parse the manifest JSON we just wrote
    ...
    stage('Extract gateways command from manifest') {
      steps { script { echo 'TODO' } }
    }
